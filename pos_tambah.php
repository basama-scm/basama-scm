<div class="row mt-3">
    <div class="col-lg-12">
        <h3>Kelola Data Penambahan Stok</h3>
        <hr>
        <div class="card mt-3">
            <div class="card-body">
                <div class="row     ">
                    <div class="col-8 border border-1">
                        ...
                        <hr>
                        <?= pesan($_GET['alert']) ?>
                        <form action="aksi.php?m=keranjang" method="POST">
                        ...
                        <table class="table table-bordered mt-3 mb-3">
                            ...
                            <tbody>
                                <?php
                                $a = "INNER JOIN tb_produk ON tb_keranjang.id_produk=tb_produk.id_produk";
                                $rows = $db->get_results("SELECT * FROM tb_keranjang $a");
                                $no = 1;
                                foreach ($rows as $row) {

                                    $jumlah[] = kali($row->harga_beli, $row->banyak);
                                ?>
                                    <tr>
                                        <td>
                                            <center><?= $no++; ?></center>
                                        ...
                                        <td align="right"><?= rupiah1(kali($row->harga_beli, $row->banyak)) ?></td>
                                        <td>
                                            <center>
                                                <a href="aksi.php?m=pos_del&ID=<?= $row->id ?>" class="btn btn-danger">Hapus</a>
                                            </center>
                                        </td>
                                    </tr>
                                <?php
                    ...
                    <div class="col-4 border border-1">
                        <h4 class="mt-3">Input Data Supplier</h4>
                        <hr>
                        <form action="aksi.php?m=belanja" method="post">
                            <?php if ($_POST) include 'aksi.php' ?>
                            <label class="mb-2">Nama Supplier</label>
                            <input type="text" class="form-control" name="nama">
                            <label class="mb-2 mt-2">Alamat Supplier</label>
                            <input type="text" class="form-control" name="alamat">
                            <label class="mb-2 mt-2">Nohp Supplier</label>
